...
_url="$(curl --silent "https://api.github.com/repos/junegunn/${_target}/releases/latest" \
  | jq -r '.assets[] | select(.name | contains('\"'linux_amd64'\"')) | select(.name | endswith('\"'tar.gz'\"')).browser_download_url')"
