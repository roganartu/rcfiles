...
class LineCount {
     ...
     */
    parseRule(text, sep) {
        let block_enable = (sep['blockstart'] && sep['blockend']);
        let doublequotes = true;
        let singlequotes = true;
        ...
        if (sep.linecomment)
            cmtlen.line = sep.linecomment.length;
        if (block_enable) {
        ...
        for (var i = 0; i < text.length; i++) {
            if (text.charAt(i) === ' ' || text.charAt(i) === '\t' || text.charAt(i) === '\r')
                continue;
...
exports.default = LineCount;
//# sourceMappingURL=LineCount.js.map